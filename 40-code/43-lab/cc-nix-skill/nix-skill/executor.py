...
class MCPExecutor:
    ...
    async def _get_session(self):
        ...
        # Return the context manager and streams
    ...
    async def list_tools(self):
        ...
        client_ctx, session = await self._get_session()
        try:
...
async def main():
    ...
    if "mcpServers" in full_config:
        servers = full_config["mcpServers"]
        if len(servers) == 0:
            print("Error: No MCP servers found in configuration", file=sys.stderr)
            sys.exit(1)
        # Use the first server (or could be made configurable)
