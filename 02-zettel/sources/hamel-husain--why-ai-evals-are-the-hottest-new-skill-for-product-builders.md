...
source-title: Why AI evals are the hottest new skill for product builders | Hamel Husain & Shreya Shankar
source-url: https://youtu.be/BsWxPI9UM4c?si=-5lKGzuhElllGKLa
...
source-creators:
  ...
  - - Hamel Husain
  - - Shreya Shankar
source-published-date: 2025-09-25
source-fetched-date: 2025-10-05
source-length-minutes: 107
topics:
  - ai
  - evals
  - product-management
...
## TLDR
- AI evals are systematic ways to measure and improve AI applications, becoming a crucial skill for product builders.
- The process involves error analysis, open coding, axial coding, and building LLM-as-judge evaluators.
- Evals help identify failure modes in AI products that aren't obvious from prompts or vibes.
- Building evals requires starting with data analysis, not jumping straight to tests.
- Evals are the new PRDs for AI products, providing automated feedback on product quality.

## Key ideas
- Evals are not just unit tests but a spectrum of systematic measurement techniques for AI applications.
- Error analysis involves manually reviewing traces and writing notes on failures to ground eval development.
- Axial coding synthesizes open codes into actionable failure mode categories.
- LLM-as-judge evaluators automate evaluation of specific failure modes, but must be validated against human judgment.
- Evals enable continuous monitoring and improvement of AI products in production.

## Detailed Summary
- Introduction to evals as a new essential skill for AI product builders, taught by Hamel Husain and Shreya Shankar.
- Definition and importance: Evals systematically measure AI application quality, replacing guesswork with data-driven iteration.
- Demo of error analysis: Reviewing real traces from a property management AI assistant to identify failures like incorrect handoffs or hallucinations.
- Open coding: Writing detailed notes on errors without categorizing initially.
- Theoretical saturation: Continuing analysis until no new error types are discovered.
- Axial coding: Using AI to categorize open codes into failure mode buckets.
- Counting and prioritization: Using pivot tables to count occurrences and prioritize fixes.
- Building evals: Code-based evals for simple checks, LLM-as-judge for complex subjective evaluations.
- LLM-as-judge process: Creating binary prompts, validating against human labels, measuring agreement.
- Application: Using evals in unit tests, CI/CD, and production monitoring.
- Debate on evals: Addressing misconceptions, importance of systematic approaches over vibes.
- Time investment: 30 minutes per week after initial setup.
- Course overview: Comprehensive training on evals methodology.
...
Upload date: 2025-09-25
Duration: 6393 seconds (1 hour 46 minutes 33 seconds)
View count: 33985
Like count: 685
Tags: []
...
(00:00) Introduction to Hamel and Shrea
...
this is one particular interaction that a
customer had with the nurture boss
application.
...
of the process that an LLM is suited
...
I love that.
Ham l pulled up a video. What do you got
going on here?
...
decide like okay, do you want an email
for that? You might be you might still
want to write an email for that because
you might be able to test that with just
code. You could just test the string.
Does it have the right formatting
potentially without running an LLM? So,
there's a
...
into eval like let me let me write some
tests and that is where things go off
the rails. Um so let's let's okay so
...
again because we kind kind of have that
cheat code from the axial coding. We
already did it.
...
the community most of the community is
beginners right or people who don't know
about evals and want to learn about it
um and it sends the wrong message
...
(File has more lines. Use 'offset' parameter to read next 2000 lines)
